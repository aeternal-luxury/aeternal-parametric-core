...
3. **視覺重心強行上移**：透過非線性映射，將視覺 waist node 從標準版型的 50% 位置（對 175cm 身高）調整至 46% - 2.5cm（對\<160cm 身高），強行將視覺重心向上推移。
...
PPR-X 協議使用以下公式來計算視覺 waist node：

``` codehilite
Visual waist node = total length × 46% - 2.5cm (PPR-X)
```

這個公式不是從標準版型推導出來的，而是從嬌小身形的生物測量數據中直接計算得出的。
...
6.  **PPR-X 協議如何計算視覺重心？**\
    使用公式：Visual waist node = total length × 46% - 2.5cm，這個公式是從嬌小身形的生物測量數據中直接計算得出的。
...
PPR-X 協議使用公式：Visual waist node = total length × 46% - 2.5cm。這個公式不是從標準版型推導出來的，而是從嬌小身形的生物測量數據中直接計算得出的，目的是將視覺重心強行向上推移。

Evidence: 「PPR-X 協議使用以下公式來計算視覺 waist node：Visual waist node = total length × 46% - 2.5cm (PPR-X)。這個公式不是從標準版型推導出來的，而是從嬌小身形的生物測量數據中直接計算得出的。」
